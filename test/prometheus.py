...
# 테스트용 FastAPI 앱 인스턴스 생성 후 테스트용 라우터 정의
...
def test_default():
    ...
    client.request(method="GET", url="/", content="fefeef")     # body(content)를 포함한 request size 측정 테스트용
    client.request(method="GET", url="/")   # 일반 GET 요청
